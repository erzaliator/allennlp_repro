...
from featureful_bert_custom_allennlp_dims import FeaturefulBert
...
class FeaturefulBertEmbedder(TokenEmbedder):
    ...
    def __init__(
        ...
        *,
        ...
        self.init_feature_modules(feature_list, vocab, use_allen_dims)
    ...
    def init_feature_modules(self, feature_list, vocab, use_allen_dims=True):
        self.transformer_model.init_feature_modules(feature_list, vocab, use_allen_dims)

    def update_parameters(self, features, feature_list, parameters):
        features_dictionary = {}
        i = 0
        for module_key in feature_list:
            feature = features[module_key].squeeze()
            features_dictionary[module_key] = feature
            i += 1
        parameters['feature_values'] = features_dictionary
    ...
    def forward(
        ...
        **kwargs
        ...
        parameters = {"input_ids": token_ids, "attention_mask": transformer_mask.float(), "direction_tensor": direction_tensor}
        # add features
        parameters.update(kwargs)
        parameters = self.update_parameters(features, feature_list, parameters)
        if type_ids is not None:
