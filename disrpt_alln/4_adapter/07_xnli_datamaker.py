...
class CustomArguments:
    ...
    Arguments misc.
    """
    train_dataset_task: Optional[str] = field(
        default=None,
        metadata={
            "help": (
                "For overwriting disrpt task."
            )
        },
    )
...
def main():
    ...
    if training_args.do_train:
        if model_args.train_language is None:
            ...
            if custom_args.train_dataset_task=='disrpt':
        ...
        if custom_args.train_dataset_task=='xnli':
    ...
    if custom_args.train_dataset_task=='disrpt':
        label_list = ClassLabel(names=list(set(train_dataset['label'])|set(test_dataset['label'])|set(valid_dataset['label'])))
        num_labels = label_list.num_classes
    else:
    ...
    if training_args.do_eval:
        if data_args.max_eval_samples is not None:
            ...
            print('2: ', len(eval_dataset))
        with training_args.main_process_first(desc="validation dataset map pre-processing"):
            print('3: ', len(eval_dataset))
        ...
        if trainer.is_world_process_zero():
            with open(output_predict_file, "w") as writer:
                ...
                for index, item in enumerate(predictions):
                    item = label_list[item] if custom_args.train_dataset_task=='xnli' else label_list.int2str(item)
